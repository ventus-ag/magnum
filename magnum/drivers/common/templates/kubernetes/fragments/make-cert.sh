...
EOF
    ...
    curl $VERIFY_CA -X GET \
        ...
        $MAGNUM_URL/certificates/$CLUSTER_UUID | jq -r '.pem' > ${CA_CERT}
    ...
    csr_req=$(jq -n --arg cluster_uuid "$CLUSTER_UUID" --arg csr "$(cat "${_CSR}")" \
    '{cluster_uuid: $cluster_uuid, csr: $csr}')
    curl $VERIFY_CA -X POST \
        -H "X-Auth-Token: $USER_TOKEN" \
        -H "OpenStack-API-Version: container-infra latest" \
        -H "Content-Type: application/json" \
        -d "$csr_req" \
        $MAGNUM_URL/certificates | jq -r '.pem' > ${_CERT}
}
