...
if [ "$(echo $ISTIO_LEAN_ENABLED | tr '[:upper:]' '[:lower:]')" == "true" ]; then

    ISTIO_LEAN_DEPLOY=/srv/magnum/kubernetes/kubernetes-istio-lean.yaml

    [ -f ${ISTIO_LEAN_DEPLOY} ] || {
        echo "Writing File: $ISTIO_LEAN_DEPLOY"
        mkdir -p $(dirname ${ISTIO_LEAN_DEPLOY})
ISTIO_LEAN_DEPLOY_YAML='apiVersion: v1
kind: Namespace
metadata:
  name: istio-system
  labels:
...
echo "$ISTIO_LEAN_DEPLOY_YAML" > $ISTIO_LEAN_DEPLOY
    }
    ...
    printf "apply ${step}\n"
    kubectl apply --validate=false -f $ISTIO_LEAN_DEPLOY
fi
